...
- Reads the status the same way: a `getStatus()` that throws, or that returns a number no response can carry (anything outside 200 to 599), answers 500 rather than taking the route's whole response down
