...
- Answers an `ApiException` whose `message` was written after construction with the constructor's own sentence naming the real status, bounded at the same 2000 characters, and never with the object, the missing field or the megabyte it was given. `ApiException.getResponse()` reads its message through the same function, so an application rendering its own envelope gets the same guarantee
