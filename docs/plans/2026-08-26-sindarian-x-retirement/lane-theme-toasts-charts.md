...
**Implementation vision:** Near-verbatim port: localStorage persistence, `.dark` class on `document.documentElement`, live `prefers-color-scheme` tracking while `'system'`, cross-tab `storage` sync. The FOUC script stays a dependency-free IIFE string. Tests: provider set/resolve/persist; the script string evaluated in a jsdom test agrees with the provider across the FULL fallback matrix, not just the happy path — every `defaultTheme` (`'system'`/`'dark'`/`'light'`) × every stored value (`'dark'`, `'light'`, `'system'`, absent, and a CORRUPTED value) × both OS preferences, which is the 30-case parity matrix as shipped. `defaultTheme="dark"` (app-cockpit) and the corrupted-storage case are load-bearing: if the script and the provider disagreed on either, the cockpit would paint one theme pre-hydration and switch after. A further 6-case matrix covers storage that THROWS (site data blocked), where both resolvers must still apply `defaultTheme`'s resolution.
