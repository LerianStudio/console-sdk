...
- `--credit` / `--credit-foreground`: the ledger-red semantic for credit amounts, DISTINCT from `--destructive` (sindarian-x drew this distinction deliberately). Derive from the palette's red range at AA contrast against `--background` in both themes; `--credit-foreground` is the on-credit text color.
- `--matched-surface` / `--unmatched-surface`: low-chroma green-tinted and red-tinted surfaces for reconciliation states, subtle enough to sit behind text at AA. Base them on `--system-success-surface` / `--system-error-surface` hues, adjusted so they read as surfaces, not alerts.
