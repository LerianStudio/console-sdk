...
**Lane:** retirement
**Depends on:** app-consignado (br-consignado-gw#123, MERGED), app-matcher (matcher#376, MERGED), app-lender (lender#183, MERGED), app-cockpit (br-sfn#143, MERGED)
...
|-------|-----------|-------|--------|
| 1 | absence verified across the five repos | 1.1 | Complete |
| 2 | console-sdk stable released from main | 2.1 | Complete |
| 3 | four apps pin the stable release | 3.1 | Complete |
| 4 | npm deprecation live + repo archived | 4.1 | Complete |
...
- [x] Done — console-sdk#143 merged 2026-08-26 (back-merge main→develop via #145, gate fixes via #146). Published dist-tags: `latest: 1.3.0`, `develop: 1.3.0-beta.1`. Note: the main release run's final `Notify Sync PR` step failed ("Need to provide at least one botToken or webhookUrl" — missing webhook secret, tracked as infra follow-up); npm publish completed before that step.
...
**Status:** Done — all four merged 2026-08-26: br-consignado-gw#124, lender#186, matcher#377 (carried the mandatory lib-streaming v3.1.0 bump), br-sfn#145 (carried the TanStack Router 1.170.32 + router-plugin 1.168.35 pairing). Every lockfile resolves `@lerianstudio/sindarian-ui@1.3.0` from the registry.
...
- [x] Done — executed 2026-08-26 via a one-off GitHub Actions workflow on a temporary console-sdk branch (`chore/deprecate-sindarian-x`, deleted after the run), using the repo's `NPM_TOKEN` secret (the service-account token) in situ. Run 32968270619. Verified from an independent machine: all 20 published versions (0.1.0–0.15.0) carry the deprecation message.
...
- [x] Done — executed 2026-08-26: description set to the pointer text, then archived. Verified: `archived: true` + pointer description.
