...
`9a9e11a`, out of 897 unit cases and 32 e2e:
...
| M14 | the pre-PR `ApiException` fallback restored (`exception.message \|\| UNCLASSIFIED`), i.e. the previous version of this branch | unit rc=1, **1 failed** / 32 passed: `answers the message it was given, even a mutated empty one`; e2e rc=0, 29 passed (at `712f004`, 33 cases, where this branch carried no `typeof` guard yet, so the mutation is the fallback alone; the same text at `2f8936e`, the head of this branch that HAS the guard, removes that too and kills **five** of its 37 cases, the four `names the real status for %s` plus `answers the message it was given, even a mutated empty one`, re-measured 2026-09-16 by the follow-up lane `fix/typed-branch-status-and-base-class`, which corrects the unheaded "six" this row carried) |
